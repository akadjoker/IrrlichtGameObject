	...
	struct SAnimatedMesh : public IAnimatedMesh
	{
		...
		virtual IMesh* getMesh(s32 frame, s32 detailLevel=255, s32 startFrameLoop=-1, s32 endFrameLoop=-1) _IRR_OVERRIDE_
		{
			if (Meshes.empty())
                ...
                //SDL_Log(" %d %d",frame,detailLevel);
            CurrentFrame = (u32)frame;
         ...
         /*   for (u32 i=0; i<MeshBuffers.size(); ++i)
            {
                ...
                int PrimitiveCount=MeshBuffers[i]->getPrimitiveCount();
                E_PRIMITIVE_TYPE primitiveType=MeshBuffers[i]->getPrimitiveType();
                MeshBuffers[i]->render(PrimitiveCount,primitiveType);
            }*/
