...
void CAnimatedMeshMD3::setHardwareMappingHint(E_HARDWARE_MAPPING newMappingHint,		E_BUFFER_TYPE buffer)
{
...
IMeshBuffer * CAnimatedMeshMD3::createMeshBuffer(const SMD3MeshBuffer* source)
{
	...
	//getTextures(textureArray, source->Shader, pos, fs, driver);
					...
					SMeshBuffer* dest)
{
	...
	for (s32 i = 0; i != source->MeshHeader.numVertices; ++i)
	{
		...
		// normal
		const core::vector3df nA(getMD3Normal(vA.normal[0], vA.normal[1]));
		const core::vector3df nB(getMD3Normal(vB.normal[0], vB.normal[1]));
