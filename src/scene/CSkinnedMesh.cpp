...
u32 CSkinnedMesh::getCurrentFrame() const
...
void CSkinnedMesh::recoverJointsFromMesh(core::array<IGameObject*> &jointChildSceneNodes)
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
		IGameObject* node=jointChildSceneNodes[i];
		SJoint *joint=AllJoints[i];



		node->Transform->setPosition(joint->LocalAnimatedMatrix.getTranslation());
		node->Transform->setRotation(joint->LocalAnimatedMatrix.getRotationDegrees());
		node->Transform->setScale(joint->LocalAnimatedMatrix.getScale());

		std::shared_ptr<IBoneValue> value = node->getComponent<IBoneValue>();
		if (value.get())
		{
            value.get()->positionHint=joint->positionHint;
            value.get()->scaleHint=joint->scaleHint;
            value.get()->rotationHint=joint->rotationHint;
		}
...
void CSkinnedMesh::transferJointsToMesh( core::array<IGameObject*> &jointChildSceneNodes)
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
		 IGameObject*  node=jointChildSceneNodes[i];
		...
		joint->LocalAnimatedMatrix.setRotationDegrees(node->Transform->getRotation());
		joint->LocalAnimatedMatrix.setTranslation(node->Transform->getPosition());
		joint->LocalAnimatedMatrix *= core::matrix4().setScale(node->Transform->getScale());

		std::shared_ptr<IBoneValue> value = node->getComponent<IBoneValue>();
		if (value.get())
		{
            joint->positionHint=value.get()->positionHint;
            joint->scaleHint   =value.get()->scaleHint;
            joint->rotationHint=value.get()->rotationHint;
            joint->GlobalSkinningSpace=(value.get()->getSkinningSpace()==EBSS_GLOBAL);
		}
	}
...
void CSkinnedMesh::transferOnlyJointsHintsToMesh( core::array<IGameObject*> &jointChildSceneNodes)
{
...
void CSkinnedMesh::addJoints(core::array<IGameObject*> &jointChildSceneNodes,IGameObject* node)
{

	//Create new joints
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
	    IGameObject* gmJoint=new IGameObject();
	    gmJoint->addComponent<IBoneValue>(i,AllJoints[i]->Name.c_str());
	    gmJoint->setName(AllJoints[i]->Name.c_str());
		jointChildSceneNodes.push_back(gmJoint);
	}
	...
	for (u32 i=0; i<jointChildSceneNodes.size(); ++i)
	{
		...
		IGameObject* bone=jointChildSceneNodes[i];
		if (parentID!=-1)
