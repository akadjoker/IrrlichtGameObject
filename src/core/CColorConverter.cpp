...
void CColorConverter::convert8BitToRGB(const u8* in, u8* out, s32 width, s32 height, const s32* palette, s32 linepad)
{
	if (!in || !out || !palette)
		return;
...
for (s32 y=0; y<height; ++y)
	{
		for (s32 x=0; x<width; ++x)
		{
			   u16 pal =   X8R8G8B8toA1R5G5B5((u32)palette[(u8)(*in)]);
			   u32 color = A1R5G5B5toA8R8G8B8(pal);
			   SColor finalColor = SColor(color);
			    out[k+0]   =(u8)finalColor.getRed();
			    out[k+1]   =(u8)finalColor.getGreen();
			    out[k+2]   =(u8)finalColor.getBlue();
			   k+=3;
			++in;
		}
